# AGENT_FULL_GUIDE — общий обзор Management UI (legacy)

> **Для публикации сервиса или проекта** используйте [AGENT_PUBLISH.md](AGENT_PUBLISH.md) — там правила публикации (мульти-домен, Authelia checklist, idempotency) и описание нового API `/api/publish/*`.
>
> Этот документ сохраняется как общий обзор системы (endpoint'ы мониторинга, релизов, инфраструктуры). Разделы про публикацию могут быть устаревшими — ориентируйтесь на `AGENT_PUBLISH.md`.

Единый обзорный документ. Для быстрого понимания что умеет Management UI в целом.
