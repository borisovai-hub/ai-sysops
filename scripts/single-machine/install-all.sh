...
echo "  - Mailu Mail Server (опционально)"
echo ""
...
echo ""
SETUP_MAIL_CHOICE=$(prompt_choice_and_save "setup_mail_choice" "Установить Mailu Mail Server? (y/n)")
MAIL_DOMAIN=""
...
if [ "$TRAEFIK_OK" = false ] || [ "$GITLAB_OK" = false ] || [ "$N8N_OK" = false ] || [ "$UI_OK" = false ] || ([ -n "$MAIL_DOMAIN" ] && [ "$MAILU_OK" = false ]); then
    echo ""
